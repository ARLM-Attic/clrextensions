...
Namespace CodeGeneration
    Public Module ExpressionUtilities
        ...
        <Untested()>
        Public Function CreateFunction(Of TArg1, TResult)(ByVal body As Func(Of Expression, UnaryExpression)) As Func(Of TArg1, TResult)
        ...
        <Untested()>
        Public Function CreateFunction(Of TArg1, TArg2, TResult)(ByVal body As Func(Of Expression, Expression, BinaryExpression)) As Func(Of TArg1, TArg2, TResult)
            Dim arg1 = Expression.Parameter(GetType(Integer), "arg1")
    ...
    Public Class MathOperators(Of T)

        <Untested()>
        Public Function Add() As Func(Of T, T, T)
            Static m_Add As Func(Of T, T, T) = CreateFunction(Of T, T, T)(AddressOf Expression.AddChecked)
            Return m_Add
        End Function

        <Untested()>
        Public Function Subtract() As Func(Of T, T, T)
